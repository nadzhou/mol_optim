"""The Step 1 gate: DQN has to beat random on QED, and hold the level it reached.

Slow — a full 5000-episode training run plus the baseline, about 100 minutes together.
`pytest -m "not slow"` skips them; the nightly run does not.
