...
otherwise shifts the canonical ranking and renames the molecule. This has to change when
Step 4 brings in real inhibitors, where stereochemistry is part of the compound.
"""
...
def canonical_hash(mol: Chem.Mol) -> str:
    ...
    mol = normalize(mol)
    # Constitution only: see the module docstring on stereochemistry.
    ranks = list(
        Chem.CanonicalRankAtoms(mol, includeChirality=False)
    )  # canonical position of each atom index

    atoms: list[tuple[int, int, int, int] | None] = [None] * mol.GetNumAtoms()
