"""BindingDB's IC50 table down to the dataset the regressor trains on.

Downloads a dated snapshot (593 MB) unless it is there, checks the MD5 BindingDB
publishes beside it, streams the 9 GB table once, and writes the SDF. Four cleaning
decisions, each silent if skipped:
...
from mol_optim import bindingdb, config, graph_key, molio

# Column positions in that table's 640, checked against its header below.
